...
/* this function read the accelerometer, gyro and mag value from 
*  the board.
*/
...
void ReadAccelGyroMag()
{
    ...
    GX = ((float)CGX) / 32.768; //32.768 is just 32768/1000 to get us our 1deg/sec value
    GY = ((float)CGY) / 32.768;
    GZ = ((float)CGZ) / 32.768;
...
int main(void)
{
    ...
    while (1)
    {
		...
		//DKW - Note - the "literal" values to be used below are degrees per second for gyro;
        //      g for accelerometer and any calibrated units for magnetometer
        
        // set gyrscope, accelerometer and magnetmeter to values read from board.
        ...
        // Call the function to update the algorithm with the latest sensor measurements.
        FusionAhrsUpdate(&fusionAhrs, gyroscope, accelerometer, magnetometer, 0.01f); 
               
        // Initialises the bias correction algorithm with application settings.      
        // Liz - I'm not sure what these value in fusionBias would be used for??? but it say to call this function
        ...
        // Not sure what you do with these results???            	
        const FusionRotationMatrix rotationMatrix = FusionQuaternionToRotationMatrix(fqc);
